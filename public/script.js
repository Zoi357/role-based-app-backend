...
async function register() {
    if (!document.getElementById('regEmail').value || !document.getElementById('regEmail').value.endsWith('@example.com'))
...
function renderDepartments() {
    ...
    if (departments.length === 0) {
        departmentsTable.innerHTML = `<tr><td colspan="4" style="text-align:center">No departments.</td></tr>`;
        return;
    }
...
function renderMyRequests() {
    ...
    if (myRequests.length === 0) {
        myRequestsTable.innerHTML = `<tr><td colspan="4" style="text-align:center">No requests.</td></tr>`;
        return;
    }
